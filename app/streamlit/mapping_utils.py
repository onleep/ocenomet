...
column_order = [
    ...
    "Кол-во зданий застройщика",
    "Год основания застройщика",
...
def map_values(material_type, flat_type, repair_type, travel_type):
    return {
        "material_type":
            value_mappings["material_type"].get(material_type, material_type),
        "flat_type":
            value_mappings["flat_type"].get(flat_type, flat_type),
        "repair_type":
            value_mappings["repair_type"].get(repair_type, repair_type),
        "travel_type":
            value_mappings["travel_type"].get(travel_type, travel_type),
    }
...
def map_dataframe(df, direction):
    ...
    for column in df.columns:
        if column in value_mappings:
            ...
            selected_mapping = mapping if direction == "to_english" else {
                v: k
