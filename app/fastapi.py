...
async def process_json(file: UploadFile = File(...)):
    ...
    offers_df['publication_at'] = pd.to_datetime(pd.to_datetime(offers_df['publication_at'], unit = 's', utc = True).dt.date)

    offers_df = offers_df.dropna(subset=['photos_count'])
    offers_df['publication_at'] = pd.to_datetime(pd.to_datetime(offers_df['publication_at'], unit = 's', utc = True).dt.date)
    
    realty_details_df['finish_year'] = realty_details_df['finish_date'].apply(lambda x: x.get('year') if isinstance(x, dict) else None)

    realty_details_df['realty_type'] = realty_details_df['realty_type'].replace('none', None)
    realty_details_df['heat_type'] = realty_details_df['heat_type'].replace('none', None)
    ...
    realty_outside_df['material_type'] = realty_outside_df['parking_type'].replace('none', None)
    ...
    # Шаг 5: Подготовка данных для возврата в ответе
    response = {
